...
class Woostify_WebFont_Loader {
	...
	public function __construct( $url = '' ) {
		...
		add_action( 'woostify_delete_fonts_folder', array( $this, 'woostify_delete_fonts_folder' ) );
	}
	...
	public function get_local_files_from_css() {
		...
		$stored     = get_site_option( 'woostify_downloaded_font_files', array() );
		...
		if ( $change ) {
			...
			update_site_option( 'woostify_downloaded_font_files', $stored );
		}
	...
	public function get_local_stylesheet_filename() {
		return apply_filters( 'woostify_local_font_file_name', 'woostify-local-fonts' );
	}
	...
	public function set_font_format( $format = 'woff2' ) {
		$this->font_format = apply_filters( 'woostify_local_google_fonts_format', $format );
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = apply_filters( 'woostify_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'woostify_local_fonts_base_url', content_url() );
		}
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'woostify_local_fonts_directory_name', 'woostify-local-fonts' );
		}
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'woostify_delete_fonts_folder' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'woostify_delete_fonts_folder' );
			}
		}
	}
...
if ( ! function_exists( 'woostify_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function woostify_get_webfont_styles( $url, $format = 'woff2' ) {
...
if ( ! function_exists( 'woostify_get_webfont_url' ) ) {
	/**
...
if ( ! function_exists( 'woostify_webfont_loader_instance' ) ) {
	/**
	 * Create instance of Woostify_WebFont_Loader class.
	 *
