	...
	class Woostify_Fonts_Helpers {
		...
		public function woostify_do_control_inline_scripts() {
			wp_localize_script(
				...
				'woostify_customize',
				array(
					'nonce' => wp_create_nonce( 'woostify_customize_nonce' ),
				)
			);
			...
			wp_localize_script(
				'woostify-typography-customizer',
				'typography_defaults',
				$this->woostify_typography_default_fonts()
			);
		}
		 ...
		 */
		public static function woostify_typography_default_fonts() {
			$fonts = array(
				'inherit',
				'System Stack',
				'Arial, sans-serif',
		...
		public function woostify_enqueue_google_fonts() {
			...
			$not_google = str_replace( ' ', '+', self::woostify_typography_default_fonts() );
			...
			if ( ! empty( $font_settings ) ) {
				...
				foreach ( $font_settings as $key ) {
					...
					$variants = $this->woostify_get_google_font_variants( $woostify_settings[ $key ], $key );
		 ...
		 */
		public static function woostify_get_all_google_fonts( $amount = 'all' ) {
		...
		public static function woostify_get_google_font_category( $font, $key = '' ) {
			...
			// Grab all of our fonts. It's a big list, so hopefully we're not even still reading.
			$fonts = self::woostify_get_all_google_fonts();
		 ...
		 *
		 * @param      string $fonts   The font.
		 */
		public function woostify_add_to_font_customizer_list( $fonts ) {
			...
			$select_fonts = $this->woostify_get_all_google_fonts( apply_filters( 'woostify_number_of_fonts', 200 ) );
		 ...
		 */
		public static function woostify_get_font_family_css( $font, $settings, $default ) {
			$woostify_settings = wp_parse_args(
